    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-4">30-Day Attendance Trend</h2>
        ...
        {/* Class Comparison */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-4">Class Attendance Comparison</h2>
          {classComparison.length === 0 ? (
            <p className="text-gray-400 text-sm text-center mt-16">No data yet</p>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={classComparison}>
                ...
                <YAxis domain={[0, 100]} tick={{ fontSize: 11 }} />
                <Tooltip formatter={(v) => `${v}%`} />
                <Bar dataKey="attendance_rate" fill="#6366f1" radius={[4, 4, 0, 0]} name="Attendance %" />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 className="text-base font-semibold text-gray-900 mb-4">
          At-Risk Students
          <span className="ml-2 text-xs font-normal text-gray-500">(below 70% attendance)</span>
        </h2>
        {atRisk.length === 0 ? (
          <p className="text-gray-400 text-sm text-center py-8">No at-risk students</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 text-left">
                  <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider rounded-l-lg">Student</th>
                  <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Sessions</th>
                  <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Present</th>
                  <th className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider rounded-r-lg">Rate</th>
                </tr>
              </thead>
